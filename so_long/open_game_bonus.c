...
/*   Updated: 2022/02/07 14:45:46 by sayar            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_put_img(char c, int x, int y, t_map *map)
{
	...
	photo = mlx_xpm_file_to_image(map->mlx, ft_pull_img(c, map), &w, &h);
...
void	ft_open_game(t_map *map)
{
	...
	mlx_hook(map->mlx_win, 10, 10, ft_destroy_window, map);
	ft_put_in_map(map);
	mlx_loop(map->mlx);
}
